...
static void rebind_real_buff_arrays_vao(glvao_t *vao, int old_buffer, int new_buffer) {
    for (int j = 0; j < hardext.maxvattrib; j++) {
        if (vao->vertexattrib[j].real_buffer == old_buffer) {
            vao->vertexattrib[j].real_buffer = new_buffer;
        }
    }
}

void rebind_real_buff_arrays(int old_buffer, int new_buffer) {
    // Iterate all VAOs, not just the active one, to avoid stale real_buffer references
    khash_t(glvao) *vaos = glstate->vaos;
    if (vaos) {
        for (khint_t k = kh_begin(vaos); k != kh_end(vaos); ++k) {
            if (!kh_exist(vaos, k)) continue;
            rebind_real_buff_arrays_vao(kh_value(vaos, k), old_buffer, new_buffer);
        }
    } else {
        rebind_real_buff_arrays_vao(glstate->vao, old_buffer, new_buffer);
    }
}
...
void APIENTRY_GL4ES gl4es_glBufferData(GLenum target, GLsizeiptr size, const GLvoid * data, GLenum usage) {
    ...
    // update binded VA across ALL VAOs to avoid stale real_buffer references
    {
        khash_t(glvao) *vaos = glstate->vaos;
        if (vaos) {
            for (khint_t k = kh_begin(vaos); k != kh_end(vaos); ++k) {
                if (!kh_exist(vaos, k)) continue;
                glvao_t *vao = kh_value(vaos, k);
                for (int i = 0; i < hardext.maxvattrib; ++i) {
                    vertexattrib_t *v = &vao->vertexattrib[i];
                    if (v->buffer == buff) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (t) {    // don't allow to remove default one
                ...
                if (k != kh_end(list)) {
                    ...
                    // Clear buffer references across ALL VAOs to avoid dangling pointers
                    {
                        khash_t(glvao) *vaos = glstate->vaos;
                        if (vaos) {
                            for (khint_t vk = kh_begin(vaos); vk != kh_end(vaos); ++vk) {
                                if (!kh_exist(vaos, vk)) continue;
                                glvao_t *vao = kh_value(vaos, vk);
                                if (vao->vertex == buff) vao->vertex = NULL;
                                if (vao->elements == buff) vao->elements = NULL;
                                if (vao->pack == buff) vao->pack = NULL;
                                if (vao->unpack == buff) vao->unpack = NULL;
                                for (int j = 0; j < hardext.maxvattrib; j++) {
                                    if (vao->vertexattrib[j].buffer == buff) {
                                        vao->vertexattrib[j].buffer = NULL;
                                        vao->vertexattrib[j].real_buffer = 0;
                                        vao->vertexattrib[j].real_pointer = 0;
                                    }
                                }
                            }
                        } else {
