...
// Resolve the absolute CPU-side source address of a vertex attrib for the
// renderlist capture (copy) path. Prevents incorrect assumption that ptr+ptr
// result in the correct memory address/values being read.
static inline const GLvoid* va_src_cpu(const vertexattrib_t *ptr) {
    return ptr->buffer
        ? (const GLvoid*)((uintptr_t)ptr->buffer->data + (uintptr_t)ptr->pointer)
        : (const GLvoid*)ptr->pointer;
}

GLvoid *copy_gl_pointer(vertexattrib_t *ptr, GLsizei width, GLsizei skip, GLsizei count) {
	float filler = 0.0f;
    return copy_gl_array_convert(va_src_cpu(ptr), ptr->type, ptr->size, ptr->stride,
...
void copy_gl_pointer_raw_noalloc(void* dest, vertexattrib_t *ptr, GLsizei width, GLsizei skip, GLsizei count) {
    copy_gl_array(va_src_cpu(ptr), ptr->type, ptr->size, ptr->stride,
...
void copy_gl_pointer_tex_noalloc(void* dest, vertexattrib_t *ptr, GLsizei width, GLsizei skip, GLsizei count) {
    //float filler = 1.0f;
    copy_gl_array_texcoord(va_src_cpu(ptr), ptr->type, ptr->size, ptr->stride,
                         /*GL_FLOAT,*/ width, skip, count, /*&filler,*/ dest);
}
...
GLfloat *gl_pointer_index(vertexattrib_t *p, GLint index) {
    ...
    uintptr_t ptr = (uintptr_t)va_src_cpu(p) + (stride * index);
